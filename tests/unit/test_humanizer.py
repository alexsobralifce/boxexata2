...
def test_greeting_morning() -> None:
    ...
    with patch("src.application.services.humanizer.datetime") as mock_dt:
        mock_dt.now.return_value = datetime(2026, 6, 11, 8, 0, 0, tzinfo=timezone.utc).astimezone(
            BRASILIA_TZ
        )
...
def test_greeting_afternoon() -> None:
    ...
    with patch("src.application.services.humanizer.datetime") as mock_dt:
        mock_dt.now.return_value = datetime(2026, 6, 11, 15, 0, 0, tzinfo=timezone.utc).astimezone(
            BRASILIA_TZ
        )
...
def test_greeting_night() -> None:
    ...
    with patch("src.application.services.humanizer.datetime") as mock_dt:
        mock_dt.now.return_value = datetime(2026, 6, 11, 21, 0, 0, tzinfo=timezone.utc).astimezone(
            BRASILIA_TZ
        )
